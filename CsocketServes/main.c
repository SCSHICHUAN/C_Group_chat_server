    ...
    while(1){
        ...
        if(conClientCount <= 0){
            ...
            for(int i =0; i<conClientCount; i++){
                long sendMsg_len = send(arrConSocket[i].socketCon, userStr, 900, 0);
//                int sendMsg_len = write(arrConSocket[i].socketCon,userStr,900);
                if(sendMsg_len > 0){
...
void *fun_thrReceiveHandler(void *socketInfo){
    ...
    while(1){
       ...
       //添加对buffer清零
        ...
        buffer_length = read(_socketInfo.socketCon,buffer,1000);
        if(buffer_length == 0){
        ...
        for(int i =0; i<conClientCount; i++){
            sleep(0.5);
            long sendMsg_len = send(arrConSocket[i].socketCon, buffer, 1000, 0);
            //                int sendMsg_len = write(arrConSocket[i].socketCon,userStr,900);
            if(sendMsg_len > 0){
                printf("向%s:%d发送成功\n",arrConSocket[i].ipaddr,arrConSocket[i].port);
            }else{
